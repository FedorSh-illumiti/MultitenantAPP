...
const cfenv_1 = __importDefault(require("cfenv"));
const http_client_1 = __importDefault(require("@sap-cloud-sdk/http-client"));
const appEnv = cfenv_1.default.getAppEnv();
const LOG = cds.log('sql');
const getSubscriptions = async (registry) => {
    try {
        // get subscriptions
        let res = await http_client_1.default.executeHttpRequest({ destinationName: 'app-registry' }, {
            method: 'GET',
...
const getCFInfo = async (appname) => {
    try {
        // get app GUID
        let res1 = await http_client_1.default.executeHttpRequest({ destinationName: 'app-cfapi' }, {
            method: 'GET',
            url: '/v3/apps?organization_guids=' + appEnv.app.organization_id + '&space_guids=' + appEnv.app.space_id + '&names=' + appname
        });
        // get domain GUID
        let res2 = await http_client_1.default.executeHttpRequest({ destinationName: 'app-cfapi' }, {
            method: 'GET',
...
const createRoute = async (subscribedSubdomain, appname) => {
    getCFInfo(appname).then(async function (CFInfo) {
        try {
            // create route
            let res1 = await http_client_1.default.executeHttpRequest({ destinationName: 'app-cfapi' }, {
                method: 'POST',
